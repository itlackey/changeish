...
@test "Remote: happy path" {
  ...
  run "$CHANGEISH_SCRIPT" --remote --api-url http://fake --api-model dummy
  [ "$status" -eq 0 ]
...
@test "Ollama: executable missing" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "ollama not found"
}
...
EOF
  ...
  run "$CHANGEISH_SCRIPT" --remote --api-url http://fake --api-model gpt-mini
  [ "$status" -ne 0 ]
...
EOF
  ...
  run "$CHANGEISH_SCRIPT" --remote --api-model gpt-mini
  [ "$status" -ne 0 ]
...
@test "Model: --api-model overrides --model for remote" {
  ...
  mock_curl "bar" ""
  export CHANGEISH_API_KEY="tok"
  run "$CHANGEISH_SCRIPT" --to HEAD --remote --api-url http://fake --model foo --api-model bar
  [ "$status" -eq 0 ]
