...
test_include_pattern_only() {
    ...
    "$CHANGEISH_SCRIPT" --include-pattern TODO.md --save-history
...
echo '{
  "choices": [
    {
      "message": {
        "content": "Generated by changeish"
      }
    }
  ]
}'
...
test_model_cli_overrides_env() {
    ...
    "$CHANGEISH_SCRIPT" --model phi --version > out.txt 2>&1
    grep -q "phi" out.txt
}
...
echo '{"choices":[{"message":{"content":"Generated by changeish"}}]}'
EOF
    chmod +x stubs/curl
    export PATH="$PWD/stubs:$PATH"
    export CHANGEISH_API_KEY=tok
    "$CHANGEISH_SCRIPT" --remote --api-url http://mock --model foo --api-model bar > out.txt 2>&1
    grep -q "Generated by changeish" out.txt
