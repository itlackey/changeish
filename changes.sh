...
insert_changelog() {
    ...
    echo "Inserting changelog into file: $file"
    # Auto-generate section_name if not provided
    if [[ "$section_name" = "auto" ]]; then

        echo "Auto-detecting section name..."

        new_version=""
        current_version=""
        # Extract version from the version file
        if [[ -f "$found_version_file" ]]; then
            #new_version=$(grep -Eoi "version[ :=\"']*[\"']?([0-9]+\.[0-9]+\.[0-9]+)" "$outfile" | head -n1 | sed -E 's/.*([0-9]+\.[0-9]+\.[0-9]+).*/\1/')
            new_version=$(grep -Eoi "[0-9]+\.[0-9]+\.[0-9]+" "$outfile" | tail -n1)
            current_version=$(grep -Eoi "[0-9]+\.[0-9]+\.[0-9]+" "$found_version_file" | head -n1)
        ...
        echo "Detected section name: $section_name"
    fi

    echo "$update_mode changelog into section '$section_name' in file '$file'"

    local file_content
    file_content="$content\n\nGenerated by changeish"

    # Escape dots for regex
    local esc_name="${section_name//./\\.}"
    local pattern="^##[[:space:]]*\\[?${esc_name}\\]?"
    ...
    update)
        if ((section_exists)); then
            echo "Updating section '$section_name' via LLM..."
            # Re-run generation targeting only that section
            # generate_changelog "$model" "$file"

            # Replace the existing section with new content
            sed -i "/$pattern/,/^## /{s/.*//;h;d;}" "$file"
            # Insert the new content
            sed -i "/$pattern/r /dev/stdin" "$file" <<<"$file_content"
        else
            echo "Error: Section '$section_name' not found for update." >&2
            exit 1
        fi
        ;;
    auto | prepend)
        if ((section_exists)); then
            # Insert before the matching section
            sed -i "/$pattern/i## $section_name\n$file_content" "$file"
        else
            # Insert after the top-level # header and before first subheading
            if grep -q '^## ' "$file"; then
                local ln
                ln=$(grep -n '^## ' "$file" | head -n1 | cut -d: -f1)
                sed -i "${ln}i## $section_name\n$file_content" "$file"
            else
                sed -i "1a## $section_name\n$file_content" "$file"
            fi
        fi
        ;;
    append)
        if ((section_exists)); then
            # Insert after the matching section and before next section or EOF
            local start
            start=$(grep -nE "$pattern" "$file" | head -n1 | cut -d: -f1)
            local next
            next=$(grep -n '^## ' "$file" | awk -v s=$start -F: '$1 > s {print $1; exit}')
            if [[ -n "$next" ]]; then
                sed -i "$((next - 1))a## $section_name\n$file_content" "$file"
            else
                sed -i "\$a## $section_name\n$file_content" "$file"
            fi
        else
            # Append at end
            echo "" >>"$file"
        ...
        ;;
    *)
        echo "Error: Invalid update mode '$update_mode'." >&2
        exit 1
        ;;
    esac
}
