...
insert_changelog() {
    local file="$1"
    local content="$2"
    local version="${section_name:-$current_version}"
    local esc_version
    ...
    if [[ -n "$existing_changelog_section" ]]; then
        ...
        case "$update_mode" in
        update | auto)
            echo "Updating existing changelog section for '$section_name' in $file"
            echo "Existing section content: $existing_changelog_section"
        ...
        prepend)
            ...
            ;;
        append)
            # Insert 'Current Changes' section after matching section
            echo "Appending 'Current Changes' section after existing section for '$section_name' in $file"
            # Find the start of the section
            local esc_section
            esc_section="$(echo "$section_name" | sed 's/[][\\/.*^$]/\\&/g')"
            local start_line end_line
            start_line=$(grep -nE "^##[[:space:]]*\\[?$esc_section\\]?" "$file" | head -n1 | cut -d: -f1 || true)
            echo "Start line for section '$section_name': $start_line"
            if [[ -z "$start_line" ]]; then
                echo "Section '$section_name' not found in $file" >&2
                exit 1
            fi
            echo "Finding end line for section '$section_name' in $file"
        ...
        echo "Adding ($update_mode) new changelog section for '$section_name' in $file"
        case "$update_mode" in
        append)
            # Add versioned section at the end
            echo -e "\n## $version\n$content" >>"$file"
            ;;
        update | auto | prepend)
            ...
            echo "First H1 line: $first_h1_line"
            if [[ -n "$first_h1_line" ]]; then
                # Insert after the first H1 line (preserving a blank line after H1 if present)
